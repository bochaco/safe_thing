...
extern "C" fn mdata_get_value_callback(ctx_ptr: *mut c_void, result: FfiResult, value_ptr: *const u8, value_len: usize, version: u64) {
    if result.error_code == -106 {
        ...
        unsafe {
            ...
            ctx.output.entry_value = String::from_utf8(value).unwrap();
            ctx.output.entry_version = version;
            println!("MutableData value retrieved {} - {}", ctx.output.entry_value, ctx.output.entry_version);
        };
    }
}
...
extern "C" fn mdata_entry_actions_callback_2insert(ctx_ptr: *mut c_void, result: FfiResult, entry_actions_h: MDataEntryActionsHandle) {
    ...
    unsafe {
        ...
        let ref key = ctx.input.entry_key;
        let ref value = ctx.input.entry_value;
...
extern "C" fn mdata_entry_actions_callback_2update(ctx_ptr: *mut c_void, result: FfiResult, entry_actions_h: MDataEntryActionsHandle) {
    ...
    unsafe {
        ...
        let ref key = ctx.input.entry_key;
        let ref value = ctx.input.entry_value;
        let version = ctx.input.entry_version + 1;
...
impl CallbackContext {
    pub fn new(safe_app: *mut App) -> CallbackContext {
        CallbackContext {
            safe_app: safe_app,
            ...
            input: ContextInput {
                ...
                entry_value: Default::default(),
                entry_version: Default::default()
            },
            output: ContextOutput {
                entry_value: Default::default(),
                entry_version: Default::default()
...
impl SAFENet {
    ...
    pub fn new_pub_mutable_data(&mut self, xor_name: [u8; 32], type_tag: u64) -> ResultReturn<MutableData> {
    ...
    #[allow(dead_code)]
    pub fn new_priv_mutable_data(&mut self, xor_name: [u8; 32], type_tag: u64) -> ResultReturn<MutableData> {
        let mut ctx = CallbackContext::new(self.safe_app);
        let ctx_ptr = &mut ctx as *mut _ as *mut c_void;
        unsafe {
    ...
    // private helper function
    fn helper_md_get_value(&mut self, mdata: MutableData, key: &str) -> ResultReturn<MDEntryValue> {
        let mut ctx = CallbackContext::new(self.safe_app);
        let ctx_ptr = &mut ctx as *mut _ as *mut c_void;
        unsafe {
        ...
        SAFENet::sleep(2000);
        Ok((ctx.output.entry_value, ctx.output.entry_version))
    }

    pub fn mutable_data_get_value(&mut self, mdata: MutableData, key: &str) -> ResultReturn<String> {
        let entry_value = self.helper_md_get_value(mdata, key)?;
        Ok(entry_value.0)
    }

    pub fn mutable_data_set_value(&mut self, mdata: MutableData, key: &str, value: &str) -> ResultReturn<()> {
        let mut ctx = CallbackContext::new(self.safe_app);
        let ctx_ptr = &mut ctx as *mut _ as *mut c_void;
        ctx.input.entry_key = String::from(key);
        ctx.input.entry_value = String::from(value);
        unsafe {
            ...
            if current_value.0.is_empty() {
                ...
                ctx.input.entry_version = current_value.1;
